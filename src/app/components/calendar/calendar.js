...
const Days = ({ date, startDate, endDate, onClick }) => {
    ...
    for (let i = 1; i <= 7; i++) {
        labels.push(<span className="label" key={Math.random()}>{moment().day(i).format("ddd")}</span>);
    }

    var firstDay = firstDayDate.day() == 0 ? 7 : firstDayDate.day();
...
class Calendar extends React.Component {
    ...
    changeDate(date) {
        ...
        this.props.selectPeriod(startDate, endDate);
    }
